...
/** Helper to retrieve supabase client + user ID, throws if unauthenticated */
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json(getMockBudgets(userId));
  } catch (e: any) {
...
export async function PUT(request: Request) {
  try {
    const { userId } = await getAuth(request);
    const { id, ...updates }: { id: string } & BudgetUpdate = await request.json();
    const updated = {
      id,
      user_id: userId,
      category: updates.category ?? 'Unknown',
      monthly_limit: updates.monthly_limit ?? 0,
