import { createServerClient } from '@supabase/ssr';
import { NextResponse } from 'next/server';
import type { TransactionInsert, TransactionUpdate } from '@/types/database';
import { getMockTransactions } from '@/lib/mockData';

const getAuth = async (request: Request) => {
  const supabase = createServerClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      cookies: {
        getAll() {
          const cookieHeader = request.headers.get('cookie');
          if (!cookieHeader) return [];
          return cookieHeader.split('; ').map((c) => {
            const [name, ...rest] = c.split('=');
            return { name, value: rest.join('=') };
          });
        },
      },
    }
  );
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error('Unauthenticated');
  return { userId: user.id };
};

export async function GET(request: Request) {
  try {
    const { userId } = await getAuth(request);
    return NextResponse.json(getMockTransactions(userId));
  } catch (e: any) {
    return NextResponse.json({ error: e.message }, { status: 401 });
  }
}

export async function POST(request: Request) {
  try {
    const { userId } = await getAuth(request);
    const body: TransactionInsert = await request.json();
    const created = {
      id: crypto.randomUUID(),
      user_id: userId,
      type: body.type,
      amount: body.amount,
      notes: body.notes ?? null,
      category: body.category ?? null,
      transaction_date: body.transaction_date ?? new Date().toISOString(),
      created_at: new Date().toISOString(),
    };
    return NextResponse.json(created, { status: 201 });
  } catch (e: any) {
    return NextResponse.json({ error: e.message }, { status: 400 });
  }
}

export async function PUT(request: Request) {
  try {
    const { userId } = await getAuth(request);
    const { id, ...updates }: { id: string } & TransactionUpdate = await request.json();
    const updated = {
      id,
      user_id: userId,
      type: updates.type ?? 'expense',
      amount: updates.amount ?? 0,
      notes: updates.notes ?? null,
      category: updates.category ?? null,
      transaction_date: updates.transaction_date ?? new Date().toISOString(),
      created_at: new Date().toISOString(),
    };
    return NextResponse.json(updated);
  } catch (e: any) {
    return NextResponse.json({ error: e.message }, { status: 400 });
  }
}

export async function DELETE(request: Request) {
  try {
    await getAuth(request);
    return NextResponse.json({ success: true });
  } catch (e: any) {
    return NextResponse.json({ error: e.message }, { status: 400 });
  }
}
